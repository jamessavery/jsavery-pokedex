...
class MainViewModelTest {
    // TODO Fix broken tests Issue-16 with Turbine
    ...
    //
    //    @Test
    //    fun `WHEN fetchPokemon succeeds THEN uiState should be Success and WHEN onLoadMore THEN
    // concatenates next batch`() = runTest {
    //        // given
    //        val nextPageTwo = 2
    ...
    //        assertTrue(currentState is PokemonListUiState.Success)
    //        assertEquals(concatenatedPokemonList, (currentState as
    // PokemonListUiState.Success).pokemonList)
    //    }
    //
    //    @Test
    //    fun `WHEN fetchPokemon succeeds THEN uiState should be Success and WHEN onLoadMore THEN
    // error is handled appropriately`() = runTest {
    //        // given
    //        val nextPageTwo = 2
    //        coEvery { mockResponse.data } returns mockPokemonList
    //        coEvery { mockResponse.next } returns nextPageTwo
    ...
    //            coEvery { mockRepository.getPokemonList(FIRST_PAGE) } returns
    // flowOf(Result.Error(expectedException))
    //
    //            // when
    //            viewModel = MainViewModel(mockRepository)
    //            advanceUntilIdle()
