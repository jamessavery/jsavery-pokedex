...
class PokemonRepositoryImpl @Inject constructor(
    ...
    override fun getPokemonList(nextPage: Int): Flow<Result<PokemonResponse>> =
        flow {
            try {
                remoteDataSource.getPokemonList(nextPage)?.let { response -> emit(Result.Success(response)) }
    ...
    override fun getPokemonDetail(id: Int): Flow<Result<Pokemon>> =
        flow {
            try {
                remoteDataSource.getPokemonDetail(id)?.let { response -> emit(Result.Success(response)) }
                    ?: emit(Result.Error(NullPointerException()))
            } catch (e: Exception) {
                if (e is CancellationException) throw e
                emit(Result.Error(e))
